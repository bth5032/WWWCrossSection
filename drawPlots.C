...
TString drawArbitraryNumberWithResidual(ConfigParser *conf){
  ...
  l1->SetTextSize(.03);
  //cout<<__LINE__<<endl;
  l1->AddEntry(hists[0], TString(hist_labels[0]+" ["+to_string(hists[0]->Integral(hists[0]->FindBin(xmin), hists[0]->FindBin(xmax-0.001)))+"]"), "pe");
  ...
  for (int i = hists.size()-1; i>=1; i--){
    l1->AddEntry(hists[i], TString(hist_labels[i]+" ["+to_string(hists[i]->Integral(hists[i]->FindBin(xmin), hists[i]->FindBin(xmax-0.001)))+"]"), "f");
  }
