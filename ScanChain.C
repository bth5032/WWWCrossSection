...
bool hasGood3l(){
  //if (printStats) { cout<<"Number of Leptons: "<<phys.nlep_VVV_cutbased_tight()<<" "; }
  ...
  if( ( conf->get("dilmass_Z_veto") == "true" ) ) {
    if ( conf->get("signal_region") == "3lep_0SFOS" ){
      ...
      if ( ( phys.lep_pdgId().at(g_lep_inds.at(0)) == -phys.lep_pdgId().at(g_lep_inds.at(1)) ) ){
        ...
        //veto if SFOS mass < 20 GeV
        if ( (phys.lep_p4().at(g_lep_inds.at(0)) + phys.lep_p4().at(g_lep_inds.at(1))).M() <= 20 ){
          numEvents->Fill(52);
          if (printFail) cout<<phys.evt()<<" :Failed dilmass gamma veto for 3 lepton region with 2SFOS pairs"<<endl;
          return false;
        }
      }
      ...
      if ( ( phys.lep_pdgId().at(g_lep_inds.at(0)) == -phys.lep_pdgId().at(g_lep_inds.at(2)) ) ){
        ...
        //veto if SFOS mass < 20 GeV
        if ( (phys.lep_p4().at(g_lep_inds.at(0)) + phys.lep_p4().at(g_lep_inds.at(2))).M() <= 20 ){
          numEvents->Fill(52);
          if (printFail) cout<<phys.evt()<<" :Failed dilmass gamma veto for 3 lepton region with 2SFOS pairs"<<endl;
          return false;
        }
      ...
      if ( ( phys.lep_pdgId().at(g_lep_inds.at(1)) == -phys.lep_pdgId().at(g_lep_inds.at(2)) ) ){
        ...
        //veto if SFOS mass < 20 GeV
        if ( (phys.lep_p4().at(g_lep_inds.at(1)) + phys.lep_p4().at(g_lep_inds.at(2))).M() <= 20 ){
          numEvents->Fill(52);
          if (printFail) cout<<phys.evt()<<" :Failed dilmass gamma veto for 3 lepton region with 2SFOS pairs"<<endl;
          return false;
        }
    ...
    if ( (phys.lep_p4().at(g_lep_inds.at(0)) + phys.lep_p4().at(g_lep_inds.at(1)) + phys.lep_p4().at(g_lep_inds.at(2))).M() <= 10){
      numEvents->Fill(53);
      if (printFail) cout<<phys.evt()<<" :Failed 3 lepton system mass cut < 10 GeV"<<endl;
      return false;
    }
  }
...
bool hasGood2l(){
  ...
  if ((conf->get("dilmass_Z_veto") == "true") && (FT == EE) ){ //only apply for EE events
    //cout<<g_lep_inds.at(0)<<" "<<g_lep_inds.at(1)<<" "<<g_nlep<<" "<<phys.nlep()<<endl;
  ...
  else if (conf->get("ignore_jet_cuts") == "invert_W_mass"){
    if (g_njets >= 2){
...
bool isOverlapJet(const LorentzVector &jet_p4){
  /*Takes in a p4 for a jet and determines whether that jet is less than MAX_DR_JET_LEP_OVERLAP for all leptons*/ 
    ...
    cout<<__LINE__<<endl; 
  #endif
  //cout<<"Looping through "<<g_nlep<<" leptons"<<endl;
  for (int i = 0; i < g_nlep; i++){
...
//===========================================
  ...
  TH1D *loose_fake_lep1pt, *loose_fake_lep2pt, *loose_fake_lep3pt;
  ...
  if (conf->get("fakerate_study") == "true"){ 
    ...
    loose_fake_lep1pt = new TH1D("loose_fake_lep1pt", "Leading lepton p_{T} for loose events with a fake", 6000, 0, 6000 );
    loose_fake_lep1pt->SetDirectory(rootdir);
    loose_fake_lep1pt->Sumw2();

    loose_fake_lep2pt = new TH1D("loose_fake_lep2pt", "Subleading lepton p_{T} for loose events with a fake", 6000, 0, 6000 );
    loose_fake_lep2pt->SetDirectory(rootdir);
    loose_fake_lep2pt->Sumw2();

    loose_fake_lep3pt = new TH1D("loose_fake_lep3pt", "Trailing lepton p_{T} for loose events with a fake", 6000, 0, 6000 );
    loose_fake_lep3pt->SetDirectory(rootdir);
    loose_fake_lep3pt->Sumw2();
  ...
  TH1D *weighted_count = new TH1D("weighted_count", "Weighted count of events", 0,0,0);
...
//===========================================
      ...
      MC_variations->Fill("R2F2",         weight  *   phys.weight_rn_r2_n2()      *  g_neventsinfile->GetBinContent(2)   /   (g_neventsinfile->GetBinContent(6)   *   phys.weight_rn_r1_n1() )  );  //Renorm Scale 2   Fac Scale 2
      MC_variations->Fill("R2F0.5",       weight  *   phys.weight_rn_r2_n0p5()    *  g_neventsinfile->GetBinContent(2)   /   (g_neventsinfile->GetBinContent(7)   *   phys.weight_rn_r1_n1() )  );  //Renorm Scale 2   Fac Scale 0.5
      MC_variations->Fill("R0.5F1",       weight  *   phys.weight_rn_r0p5_n1()    *  g_neventsinfile->GetBinContent(2)   /   (g_neventsinfile->GetBinContent(8)   *   phys.weight_rn_r1_n1() )  );  //Renorm Scale 0.5 Fac Scale 1
      ...
      MC_variations->Fill("alpha_s ",   weight  *   phys.weight_alphas_down()     *  g_neventsinfile->GetBinContent(2)   /   (g_neventsinfile->GetBinContent(14)  *   phys.weight_rn_r1_n1() )  ); //alpha_s down
      ...
      */
      ...
      if (conf->get("fakerate_study") == "true"){ 
        ...
        if (loose_lep_index != -1){
          weighted_count->Fill("Only Loose" ,weight); //Bin 3
          if (conf->get("FR_closure_study") == "true"){
          ...
          if (c == TT_fake || c == T_fake || c == L_fake){
            loose_fake_lep1pt->Fill(phys.lep_pt().at(g_lep_inds[0]),weight);
            loose_fake_lep2pt->Fill(phys.lep_pt().at(g_lep_inds[1]),weight);
            if (g_nlep > 2) loose_fake_lep3pt->Fill(phys.lep_pt().at(g_lep_inds[2]),weight);  
          }
          ...
          weighted_count->Fill("Only Tight" ,weight); //Bin 2
          #ifdef DEBUG 
            cout<<__LINE__<<endl; 
          #endif
          if ( c == TTT_fake ){
            ...
            tight_fake_lep3pt->Fill(phys.lep_pt().at(g_lep_inds[2]), weight);
          }
          else if ( c == TT_fake ){
            tight_fake_lep1pt->Fill(phys.lep_pt().at(g_lep_inds[0]), weight);
            tight_fake_lep2pt->Fill(phys.lep_pt().at(g_lep_inds[1]), weight);    
...
//=======================================
  ...
  if (conf->get("fakerate_study") == "true"){ 
    ...
    loose_fake_lep1pt->Write();
    loose_fake_lep2pt->Write();
    loose_fake_lep3pt->Write();
